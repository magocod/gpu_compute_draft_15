use opencl_collections::utils::from_buf_u8_to_vec_i16;
...
use opencl_collections_examples::example_save_fs_to_map::utils::{
    copy_directory_to_map, from_vec_i16_to_vec_u8, get_map_summary,
    read_from_fs_and_single_write_to_map,
};
use std::path::Path;
use std::thread::available_parallelism;
use std::time::Instant;

pub type BaseType = i16;
...
fn main() {
    ...
    let cs = copy_directory_to_map::<BaseType>(
        total_threads,
        &base_path,
        system_8gb.clone(),
        system_4gb.clone(),
        ...
        &map_4gb_src,
        read_from_fs_and_single_write_to_map,
        from_buf_u8_to_vec_i16,
        from_vec_i16_to_vec_u8,
    );
    println!("{:#?}", cs);
